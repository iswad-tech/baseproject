makeBackupOfDb() {
    ...
    docker exec main-db-1 pg_dump devdb -U devuser > $addr
...
restoreDb() {
    ...
    docker exec main-db-1 psql -U devuser -d devdb -c "DROP SCHEMA public CASCADE; CREATE SCHEMA public;"
    docker exec main-db-1 psql -U devuser -d devdb -f $addr
